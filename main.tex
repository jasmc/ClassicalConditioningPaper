...
\newcommand{\includeoptionalfigure}[2]{%
  \IfFileExists{#2}{%
    \includegraphics[width=#1\linewidth]{#2}%
  }{%
    \PackageWarning{paper}{Figure file `#2' not found; figure omitted}%
  }%
}
...
\input{sections/3_Results_1.tex}

% Draft Results inputs: enable after their text and figure references are finalized.
% \input{sections/4_Results_2.tex}
% \input{sections/5_Results_3.tex}
% \input{sections/6_Results_4.tex}
% \input{sections/7_Results_5.tex}
...
\begin{figure}
  \begin{fullwidth}
    ...
    \includeoptionalfigure{0.95}{Sections/Figures/Fig1/Fig1_test1.png}

    \caption{\textbf{An optovin-based classical-conditioning assay and single-fish behavioral readout.} (A) Experimental setup, showing the head-fixed larva with eyes and tail free, the visual conditioned stimulus (CS), basal illumination, and the optovin-mediated unconditioned stimulus (US). (B) Protocol structure and CS--US contingencies for Delay, 3sTrace, 10sTrace, and randomized unpaired-control groups. (C) Tail angle over time for representative movements, illustrating the raw kinematic signal. (D) Conversion of tail-angle data to tail-movement vigor and baseline-normalized vigor. (E--G) Trial-by-time vigor heatmaps for representative Delay (E), 3sTrace (F), and unpaired-control (G) fish. The optovin-evoked US response is characterized in supplementary material. Panel assignments, sample sizes, and statistical information will be finalized with the figure artwork.
    }
    \label{fig:fig1}
  \end{fullwidth}
\end{figure}
...
\begin{figure}
  \begin{fullwidth}
    \includeoptionalfigure{0.85}{Sections/Figures/Fig2/Fig2_test1.png}
    \caption{\textbf{Population-level evidence for conditioned suppression in Delay and trace conditioning.} (A--C) CS-aligned scaled-vigor heatmaps for each matched cohort pair: unpaired control versus Delay (A), unpaired control versus 3sTrace (B), and unpaired control versus 10sTrace (C). (D--F) Condition-specific CR ratios, calculated as response vigor relative to baseline vigor, for Pre-Train, Early Test, and Late Test trial blocks in the same cohort pairs. (G--I) Trial-by-trial CR-ratio trajectories across pre-training, training, and testing for the same cohort pairs. The 10sTrace condition is retained as a main comparison but shows no clear population-level evidence of conditioning under the present protocol. Sample sizes, response windows, and statistical tests will be finalized with the figure artwork.
    }
    \label{fig:fig2}
  \end{fullwidth}
\end{figure}
...
\begin{figure}
  \begin{fullwidth}
    \includeoptionalfigure{0.75}{Sections/Figures/Fig3/Fig3_test1.png}
    \caption{\textbf{Learner classification captures heterogeneity in conditioned suppression.} (A) Classification workflow, from behavioral eligibility through the prespecified continuous learning score and threshold. (B) Score distributions for Delay, 3sTrace, and matched controls before binary classification. (C) Threshold uncertainty, false-positive control assignments, and classification stability. (D) Classifier-positive fractions by condition. (E) Paired Pre-Train-to-Early-Test changes for all eligible larvae. (F) Representative positive, negative, intermediate, and borderline trajectories. (G) All-eligible-fish validation of the population-level result. Exact panels and statistics will be finalized with the classification analysis.
    }
    \label{fig:fig3}
  \end{fullwidth}
\end{figure}
...
\begin{figure}
  \begin{fullwidth}
    \includeoptionalfigure{0.65}{Sections/Figures/Fig4/Fig4_test1.png}
    \caption{\textbf{Pooled learner responses characterize conditioned-response dynamics.} (A) Delay catch-trial and/or early-test dynamics aligned to CS onset. (B) Equivalent 3sTrace dynamics, including the trace interval. (C) Responses aligned to expected US onset. (D) Prespecified timing metrics with uncertainty. (E) Direct Delay--3sTrace comparison of response timing, duration, and trace-interval suppression. (F) Corresponding traces from all eligible larvae. (G) Control and robustness analyses using alternative windows or metrics. Predictive-timing language will be used only if the formal timing comparisons support it.
    }
    \label{fig:fig4}
  \end{fullwidth}
\end{figure}
